...
public class Pedidos extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String user = request.getParameter("cli");
        List<Pedido_Venta> lstpedidos = new PdoVtaDao().litarPedidtos(user);
        Usuario objuser = new UsuarioDao().obtenerUsuario(user);
        request.setAttribute("user", objuser);
        request.setAttribute("lstpedidos", lstpedidos);
        RequestDispatcher dispatcher = request.getRequestDispatcher("/Admin_MantClientePedidos.jsp");
